...
//
// Coins and capsule keys never convert into each other. Keys are earned by
// playing well and buy pulls; coins are what monsters are worth and buy
// nothing but more monsters. Letting food logging turn into gambling stakes is
// exactly the door that keeps shut.
...
export function recordCoins(
  ...
  try {
    if (amount < 0 && coinBalance(playerId) + amount < 0) {
